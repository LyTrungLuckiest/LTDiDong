...
public class GoogleSignInHelper {
    ...
    public void signInWithGoogle(Activity activity, ActivityResultLauncher<Intent> googleSignInLauncher) {
        GoogleSignInOptions gso = new GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN)
                .requestIdToken(activity.getString(R.string.default_web_client_id)) // Replace with actual Web Client ID
                .requestEmail()
                .build();

        mGoogleSignInClient = GoogleSignIn.getClient(activity, gso);
        Intent signInIntent = mGoogleSignInClient.getSignInIntent();
        googleSignInLauncher.launch(signInIntent);  // Launch Google Sign-In Intent
    }
